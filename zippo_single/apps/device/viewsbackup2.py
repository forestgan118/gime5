...
class StatusView(View):
    ...
    def post(self, request):
        ...
        if date_from > date_to:
            ...
            '''
        #print (status)
        url_request="/device/status/"+str(region)+"/"+str(city)+"/"+str(store)+"/"
        #print (url_request)
...
class StatusAuthView(View):
    ...
    def get(self, request,param1,param2,param3):
        ...
        if city_id!=0 and store_id!=0:
            ...
            status = DeviceStatus.objects.filter(store=store_id.values('name'),time__range=(date_from, date_to)).values('online_status','add_time','store','city','region')
        elif city_id!=0 and store_id==0:
            #data_list=Store.objects.filter(cityname=city_id.values('name')).values('name','cityname','regionname')
            status = DeviceStatus.objects.filter(city=city_id.values('name'),time__range=(date_from, date_to)).values('online_status','add_time','store','city','region')
            print (data_list)
        elif city_id==0 and store_id==0:
            status = DeviceStatus.objects.filter(region=region_id.values('name'),time__range=(date_from, date_to)).values('online_status','add_time','store','city','region')
...
def mac_data():
    ...
    if dic5:
        for key in dic5:
            ...
            wifiprobeData_day.objects.create(device_id=key,wifi_3m_num=dic5[key],wifi_1m_num=num1,store=store_select,city=city_select,region=region_select,wifi_3m_num_total=wifi3_total['nums'],wifi_1m_num_total=wifi1_total['nums1'])
        ...
        if store_di:
            for x in range(len(store_di)):
                ...
                if wifi1_total['nums1']==None:
                    wifi1_total['nums1']=0
                wifiprobeData_day.objects.create(device_id=device_di[x],wifi_3m_num=0,wifi_1m_num=0,store=store_di[x],city=city_di[x],region=region_di[x],wifi_3m_num_total=wifi3_total['nums'],wifi_1m_num_total=wifi1_total['nums1'])
        ...
        for x in range(len(store_name)):
            ...
            if wifi1_total['nums1']==None:
                wifi1_total['nums1']=0
            wifiprobeData_day.objects.create(device_id=device_id_add[x],wifi_3m_num=0,wifi_1m_num=0,store=store_name[x],city=city_name[x],region=region_name[x],wifi_3m_num_total=wifi3_total['nums'],wifi_1m_num_total=wifi1_total['nums1'])
    ...
    if store_name:
        for i in range(len(store_name)):
            ...
            if store_name[i] in store_wifi:
                ...
                total_num_di=SaleProduct_day.objects.filter(store=store_name[i]).aggregate(nums1=Sum('quantity_pro'))
                if total_num_di['nums1']==None:
                    total_num_di['nums1']=0
                total_sum_di=SaleProduct_day.objects.filter(store=store_name[i]).aggregate(nums1=Sum('sum_pro'))
                if total_sum_di['nums1']==None:
                    total_sum_di['nums1']=0
                total_acc_sum_di=SaleProduct_day.objects.filter(store=store_name[i]).aggregate(nums1=Sum('sum_acc'))
                if total_acc_sum_di['nums1']==None:
                    total_acc_sum_di['nums1']=0
                total_acc_num_di=SaleProduct_day.objects.filter(store=store_name[i]).aggregate(nums1=Sum('quantity_acc'))
                if total_acc_num_di['nums1']==None:
                    total_acc_num_di['nums1']=0
                
                day_time=now().date() + timedelta(days=0)  #获取今天时间       date = now().date() + timedelta(days=-1) #昨天
                time_stamp=time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(time.time()))#获取系统当前时间戳
                
                SaleProduct_day.objects.create(store=str(store_name[i]),time=day_time,quantity_pro=int(quantity_day[i]),price_pro=price_day[i],sum_pro=float(sum_day[i]),add_time= time_stamp,quantity_total_pro=total_num_di['nums1'],sum_total_pro=total_sum_di['nums1'],wifi_1m_num=wifi1_select,wifi_3m_num=wifi3_select,excellent_num=execllect_select,good_num=good_select,unsatisfy_num=unsatisfy_select,price_acc=price_acc_select,quantity_acc=quantity_acc_select,quantity_total_acc=total_acc_num_di['nums1'],sum_acc=sum_acc_select,sum_total_acc=total_acc_sum_di['nums1'],city=city_name[i],region=region_name[i])  #存入当天数据
            else:
                ...
                day_time=now().date() + timedelta(days=0)  #获取今天时间       date = now().date() + timedelta(days=-1) #昨天
                time_stamp=time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(time.time()))#获取系统当前时间戳
                SaleProduct_day.objects.create(store=str(store_name[i]),time=day_time,quantity_pro=0,price_pro=0,sum_pro=0,add_time= time_stamp,quantity_total_pro=total_num_di['nums1'],sum_total_pro=total_sum_di['nums1'],wifi_1m_num=0,wifi_3m_num=0,excellent_num=0,good_num=0,unsatisfy_num=0,price_acc=0,quantity_acc=0,quantity_total_acc=total_acc_num_di['nums1'],sum_acc=0,sum_total_acc=total_acc_sum_di['nums1'],city=city_name[i],region=region_name[i])  #存入当天数据
    ...
    for i in range(len(cityname)):
        total_num_di=SaleProduct_day.objects.filter(city=cityname[i]).aggregate(nums1=Sum('quantity_pro'))
        if total_num_di['nums1']==None:
            total_num_di['nums1']=0
        total_sum_di=SaleProduct_day.objects.filter(city=cityname[i]).aggregate(nums1=Sum('sum_pro'))
        if total_sum_di['nums1']==None:
            total_sum_di['nums1']=0
        total_acc_sum_di=SaleProduct_day.objects.filter(city=cityname[i]).aggregate(nums1=Sum('sum_acc'))
        if total_acc_sum_di['nums1']==None:
            total_acc_sum_di['nums1']=0
        total_acc_num_di=SaleProduct_day.objects.filter(city=cityname[i]).aggregate(nums1=Sum('quantity_acc'))
        if total_acc_num_di['nums1']==None:
            total_acc_num_di['nums1']=0
        SaleProduct_day_city.objects.create(time=day_time,quantity_pro=int(quantity_pro[i]),price_pro=price_pro[i],sum_pro=float(sum_pro[i]),add_time= time_stamp,quantity_total_pro=total_num_di['nums1'],sum_total_pro= total_num_di['nums1'],wifi_1m_num=wifi_1m_num[i],wifi_3m_num=wifi_3m_num[i],excellent_num=excellent_num[i],good_num=good_num[i],unsatisfy_num=unsatisfy_num[i],price_acc=price_acc[i],quantity_acc=quantity_acc[i],quantity_total_acc=total_acc_num_di['nums1'],sum_acc=sum_acc[i],sum_total_acc=total_acc_sum_di['nums1'],city=cityname[i],region=region[i])  #存入当天数据
    cityname.clear()
    quantity_pro.clear()
    price_pro.clear()
    sum_pro.clear()
    ...
    for i in range(len(regionname)):
        total_num_di=SaleProduct_day.objects.filter(region=regionname[i]).aggregate(nums1=Sum('quantity_pro'))
        if total_num_di['nums1']==None:
            total_num_di['nums1']=0
        total_sum_di=SaleProduct_day.objects.filter(region=regionname[i]).aggregate(nums1=Sum('sum_pro'))
        if total_sum_di['nums1']==None:
            total_sum_di['nums1']=0
        total_acc_sum_di=SaleProduct_day.objects.filter(region=regionname[i]).aggregate(nums1=Sum('sum_acc'))
        if total_acc_sum_di['nums1']==None:
            total_acc_sum_di['nums1']=0
        total_acc_num_di=SaleProduct_day.objects.filter(region=regionname[i]).aggregate(nums1=Sum('quantity_acc'))
        if total_acc_num_di['nums1']==None:
            total_acc_num_di['nums1']=0
        SaleProduct_day_region.objects.create(time=day_time,quantity_pro=int(quantity_pro[i]),price_pro=price_pro[i],sum_pro=float(sum_pro[i]),add_time= time_stamp,quantity_total_pro=total_num_di['nums1'],sum_total_pro=total_sum_di['nums1'],wifi_1m_num=wifi_1m_num[i],wifi_3m_num=wifi_3m_num[i],excellent_num=excellent_num[i],good_num=good_num[i],unsatisfy_num=unsatisfy_num[i],price_acc=price_acc[i],quantity_acc=quantity_acc[i],quantity_total_acc=total_acc_num_di['nums1'],sum_acc=sum_acc[i],sum_total_acc=total_acc_sum_di['nums1'],region=regionname[i])  #存入当天数据
