...
def mac_data():
    ...
    '''
    day_time=now().date() + timedelta(days=-2)
    year=day_time[0]
    month=day_time[1]
    day=day_time[2]
    '''
    ...
    for x in master:
        detail_data=DetailInfo.objects.filter(utime__year=year,utime__month=month,utime__day=day,mid=x['mid']).values("mac","range","utime").order_by('utime')
        ...
        #print("dict=",dict)
        day_time=now().date() + timedelta(days=-4)  #获取今天时间       date = now().date() + timedelta(days=-1) #昨天
        if dict:
            n0=dict[0] #记录无效客户数量
            ...
            if wifi1_total['nums1']==None:
                wifi1_total['nums1']=0
            wifiprobeData_day.objects.create(device_id=device_select,wifi_3m_num=n5,wifi_1m_num=n,store=store_select,city=city_select,region=region_select,province=province_select,wifi_3m_num_total=wifi3_total['nums'],wifi_1m_num_total=wifi1_total['nums1'],time=day_time)
        #print ("n=",n)
        #print ("n5=",n5)
        maclist1=[]
        u_time=[]
    ...
    if satisfy_data :
        for satis_data in satisfy_data:   #字典遍历
            ...
            sy_device_id.append(float(satis_data['device_id']))
